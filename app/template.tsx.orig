...
export default function Template({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (previousPathname && pathname !== previousPathname) {
      // Simple heuristic: if path depth increases, it's forward; otherwise backward
  ...
  const pageVariants = {
    initial: {
      opacity: 0,
      x: navigationDirection === 'forward' ? 20 : -20,
      scale: 0.98,
    },
    animate: {
      opacity: 1,
  ...
  // Variants for individual content elements
  const itemVariants = {
  ...
  // Show loading indicator during route changes on slow networks
    ...
    <>
        ...
        >
          ...
          >
            <div 
              ...
              style={{
                backgroundImage: `radial-gradient(circle at 25px 25px, rgba(220, 38, 38, 0.1) 2%, transparent 0%), radial-gradient(circle at 75px 75px, rgba(220, 38, 38, 0.1) 2%, transparent 0%)`,
          ...
          >
            ...
            <motion.div
              ...
              className="fixed top-20 right-10 w-72 h-72 bg-gradient-to-br from-red-600/5 to-red-800/5 rounded-full blur-3xl -z-10"
            />
            <motion.div
              variants={itemVariants}
              className="fixed bottom-20 left-10 w-96 h-96 bg-gradient-to-tr from-red-600/5 to-red-800/5 rounded-full blur-3xl -z-10"
            />
      ...
      <script
        dangerouslySetInnerHTML={{
          __html: `
            // Play subtle transition sound on route changes
